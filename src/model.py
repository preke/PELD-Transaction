...
class Emo_Generation(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.mood_to_hidden = Dense(3, config.hidden_size, config.hidden_size * 0.5)
        self.hidden_resize = Dense(config.hidden_size, config.hidden_size, config.hidden_size * 0.5)
    ...
    def forward(self, input_ids, attn_masks, uttr_vad, personality, init_mood):
        ...
        emo_embedding = torch.cat([self.mood_to_hidden(response_mood), self.hidden_resize(bert_hidden)]) + self.personality_to_hidden(personality)
